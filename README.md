...
> 最常见的一类非托管资源如系统文件句柄（文件流），系统的窗口句柄，网络连接（网络流），数据库连接，画刷，图标，打印机资源等。（在windows系统中的内存管理一般会将当前处于空闲状态的对象的内存释放掉，当需要访问的时候再重新提交分配物理内存，从而导致对象的物理地址是变化的。句柄是一种指向指针的指针，由于这些非托管资源使用句柄进行物理内存中对象的管理，而这些非托管对象是有状态的、持续不中断的对象资源，所以使用句柄进行内存的管理，从而不受.NET中GC的管理，这种间接访问对象的模式增强了系统对引用对象的控制）。操作系统在管理这些句柄（所有系统对象的动态地址，其中包括系统的每张图片，每个IO资源，每个窗口等）。当一个进程被初始化时,系统要为它分配一个句柄表（内核对象）。
...
　　2． 减少Object实例，从而减少GC的调用（尽量用Struct、enum值类型替代Class；或者使用Span和Record类型）
  
　　3． 共享内存（找到相似属性，构建字典，同样要防止内存泄漏，在适当的阶段进行GC）
  
　　4． 使用高速缓存（例如二维数组的内外遍历）
...
[Volatile的原理与使用](https://www.cnblogs.com/94cool/archive/2012/08/20/2647215.html)

该关键字用来修饰字段，在很多语言中都有使用这个关键字，用来实现线程间数据同步，确保该字段任何时刻都呈现最新的值。
多线程同时访问普通的字段时，会在每个线程中创建一个该字段的变量拷贝，每个线程访问自己的memory，然后从自己cache中读取操作该变量。
多线程访问由Volatile修饰的字段时，则直接访问该字段所在memory，而不是从cache中读取，且该字段不受编译器优化，所以更消耗资源。
