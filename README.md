...
最常见的一类非托管资源如系统文件句柄（文件流），系统的窗口句柄，网络连接（网络流），数据库连接，画刷，图标，打印机资源等。（在windows系统中的内存管理一般会将当前处于空闲状态的对象的内存释放掉，当需要访问的时候再重新提交分配物理内存，从而导致对象的物理地址是变化的。由于这些非托管资源使用句柄进行物理内存中对象的管理，尔这些非托管对象是有状态的、持续不中断的对象资源，所以使用句柄进行内存的管理，从而不受.NET中GC的管理）
